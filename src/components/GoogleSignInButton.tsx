"use client";

import Link from "next/link";
import { signIn } from "next-auth/react";

/**
 * Continuing with Google creates an account on the spot, with no separate
 * consent step in the OAuth flow - so this disclosure has to live right
 * here, not just on whichever page happens to render the button, or a
 * first-time Google sign-in from /login would create an account with no
 * Terms/Privacy mention at all.
 */
export function GoogleSignInButton({ callbackUrl }: { callbackUrl?: string }) {
  return (
    <div>
      <button
        type="button"
        onClick={() => signIn("google", { callbackUrl: callbackUrl ?? "/" })}
        className="focus-ring flex w-full items-center justify-center gap-2.5 rounded-full border border-border-subtle bg-white px-4 py-2.5 text-sm font-semibold text-foreground transition hover:bg-surface-muted"
      >
        <svg className="h-4.5 w-4.5" viewBox="0 0 48 48" aria-hidden>
          <path
            fill="#FFC107"
            d="M43.6 20.5H42V20H24v8h11.3C33.7 32.9 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.5 6.1 29.5 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.7-.4-3.5z"
          />
          <path
            fill="#FF3D00"
            d="M6.3 14.7l6.6 4.8C14.5 16 18.9 13 24 13c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.5 6.1 29.5 4 24 4c-7.7 0-14.3 4.3-17.7 10.7z"
          />
          <path
            fill="#4CAF50"
            d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3C29.3 35.4 26.8 36 24 36c-5.3 0-9.7-3.1-11.3-7.6l-6.5 5C9.6 39.6 16.2 44 24 44z"
          />
          <path
            fill="#1976D2"
            d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.2-4.3 5.5l6.3 5.3C39.9 36.8 44 31 44 24c0-1.3-.1-2.7-.4-3.5z"
          />
        </svg>
        Continue with Google
      </button>
      <p className="mt-2 text-center text-xs text-stone-500">
        By continuing, you agree to our{" "}
        <Link href="/legal/terms" className="font-medium text-brand-700 hover:underline">
          Terms
        </Link>{" "}
        and{" "}
        <Link href="/legal/privacy" className="font-medium text-brand-700 hover:underline">
          Privacy Policy
        </Link>
        .
      </p>
    </div>
  );
}
